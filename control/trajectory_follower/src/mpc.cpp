  ...
  const geometry_msgs::msg::Pose & current_pose, MPCData * data)
{
  ...
  *m_steer_prediction_prev = data->predicted_steer;
  ...
  max_prediction_time = 5.0 / m_param.prediction_horizon;
  ...
  const double target_time
    [&]() {
      for (size_t i = 0; i < input.relative_time.size(); ++i) {
        ...
        const double segment_dist = std::hypot(input.at(i).x - input.at(i - 1).x, input.at(i).y - input.at(i - 1).y);
        sum_dist += segment_dist;
        if (5.0 < sum_dist && i == input.relative_time.size() - 1) {
          const double prev_sum_dist = sum_dist - segment_dist;
          const double ratio = (5.0 - prev_sum_dist) / segment_dist;

          return input.relative_time.at(i - 1) + (input.relative_time.at(i) - input.relative_time.at(i - 1)) * ratio;
        }
      }
    }();

  const double dt = target_time / m_param.prediction_horizon;
  const double max_dt = std:max(dt, m_param.prediction_dt);
