...
visualization_msgs::msg::MarkerArray getBoundsCandidatesLineMarkerArray(
  const std::vector<ReferencePoint> & ref_points, std::vector<std::vector<Bounds>> & bounds_candidates, const double r, const double g, const double b,
  ...
  const VehicleParam & vehicle_param, const bool is_showing_debug_detail)
{
  ...
  appendMarkerArray(
    getBoundsCandidatesLineMarkerArray(
      debug_data_ptr->ref_points, debug_data_ptr->sequential_bounds_candidates, 0.2, 0.99, 0.99, vehicle_param.width,
      debug_data_ptr->mpt_visualize_sampling_num),
    &vis_marker_array);
