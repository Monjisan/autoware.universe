...
: TildeNode("OccupancyGridMapOutlierFilter", options)
{
  /* params */
  ...
  pointcloud_pub_ = create_tilde_publisher<PointCloud2>("~/output/pointcloud", rclcpp::SensorDataQoS());
...
: node_(node)
{
  outlier_pointcloud_pub_ = node.create_tilde_publisher<PointCloud2>(
    "~/output/debug/outlier/pointcloud", rclcpp::SensorDataQoS());
  low_confidence_pointcloud_pub_ = node.create_tilde_publisher<PointCloud2>(
    "~/output/debug/low_confidence/pointcloud", rclcpp::SensorDataQoS());
  high_confidence_pointcloud_pub_ = node.create_tilde_publisher<PointCloud2>(
